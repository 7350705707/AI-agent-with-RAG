...
Write-Host "============================================================"
Write-Host ""
Write-Host "Copy ALL of the following to the offline PC (USB / share):"
Write-Host ""
Write-Host "  1.  $ExportFile          <- Docker image"
Write-Host "  2.  The entire project folder, INCLUDING:"
Write-Host "        backend\chroma_db\        <- RAG vector embeddings  (REQUIRED for RAG)"
Write-Host "        backend\knowledge_files\ <- original uploaded docs  (REQUIRED for RAG)"
Write-Host "        backend\chat_history.db  <- conversation history"
Write-Host "        backend\.jwt_secret      <- auth secret key"
Write-Host ""
Write-Host "  NOTE: If you skip chroma_db and knowledge_files the RAG search"
Write-Host "        will return no results on the offline PC."
Write-Host ""
