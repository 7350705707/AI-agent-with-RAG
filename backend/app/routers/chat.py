...
from app.agents.chat_agent import run_chat, run_chat_stream
...
from app.utils.analytics import track_message
    ...
    def _run() -> None:
        ...
        try:
            for kind, data, sources in run_agentic_chat_stream(
                ...
                temp_file_ids=body.file_ids or [],
            ):
                if stop_event.is_set():
                    break
                if kind == "thinking":
    ...
    async def event_stream():
        ...
        try:
            ...
            if full_response and not stop_event.is_set():
                add_message(conv_id, "assistant", full_response, sources=cited_sources)
                append_exchange(conv_id, body.message, full_response)
                track_message(user_id, "chat", body.message)
