"""Markdown-based conversation history store with relevance-search recall.

Each conversation gets a ``<conversation_id>.md`` file in ``backend/history/``.
Every Q&A turn is appended as a headed block. On the next query the store
scores every past exchange against the new query using Jaccard word-overlap
...
def _history_path(conversation_id: str) -> Path:
    """Return the filesystem path for a conversation's history markdown file.

    Naming convention for *new* files:
        ``YYYY-MM-DD_<title-slug>_<conv-id-prefix8>.md``

    Existing legacy files (bare UUID) are found and returned unchanged so
    old conversations continue to work without migration.
    """
    safe = re.sub(r"[^a-zA-Z0-9\-]", "_", conversation_id)
    legacy = HISTORY_DIR / f"{safe}.md"

    # Return existing legacy file (bare-UUID name) — backward compat
    if legacy.exists():
        return legacy

    # Check if a rich-named file was already created for this conversation
    short_id = conversation_id[:8].lower()
    for existing in HISTORY_DIR.glob(f"*_{short_id}.md"):
        return existing

    # Build a descriptive name for a new file
    date_str = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    try:
        from app.database import _connect  # lazy import to avoid circular deps
        conn = _connect()
        row = conn.execute(
        ...
        if row and row["title"]:
            title_slug = re.sub(r"[^a-zA-Z0-9]+", "-", row["title"].lower().strip())[:40].strip("-")
        else:
            title_slug = "conversation"
    except Exception:
        title_slug = "conversation"

    return HISTORY_DIR / f"{date_str}_{title_slug}_{short_id}.md"
